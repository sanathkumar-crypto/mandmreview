...
def get_user_role(user_email: str) -> Optional[str]:
    """
    Get user role from RADAR API using user email.
    ...
    try:
        ...
        if isinstance(user_data, list) and len(user_data) > 0:
            role = user_data[0].get('role') or user_data[0].get('jobTitle') or user_data[0].get('title')
        elif isinstance(user_data, dict):
            role = user_data.get('role') or user_data.get('jobTitle') or user_data.get('title')
        else:
            logger.warning(f"Unexpected response format from RADAR API: {type(user_data)}")
            return None
